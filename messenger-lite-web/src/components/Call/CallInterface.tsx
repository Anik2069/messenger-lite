...
const CallInterface = ({ callId }: { callId: string }) => {
    ...
    // On Mount
    useEffect(() => {
        if (initialLoading || !user || hasInitialized.current) return;
        ...
        const { callId: payloadCallId,
            ...
            token,
            CALL_SECRET,
            isCaller } = base64UrlDecode(payload);
        ...
        if (!payloadCallId || !type || !toUserIds || !token || !CALL_SECRET) {
            console.error("Payload validation failed! Missing properties:", { payloadCallId, type, toUserIds, token, CALL_SECRET });
            endCall();
            return;
        }
